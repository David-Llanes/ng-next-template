...
@Component({
  ...
  template: `
    ...
    ></div>
    <div
      class="group-data[mode=static]:group-data-[layout=row]:group-data-[variant=floating]:group-data-[state=expanded]:group-data-[side=left]:border-r-0 fixed top-0 bottom-0 left-0 z-10 h-svh w-[var(--sidebar-width)] transition-[left,right,width,padding] duration-200 ease-linear *:overflow-hidden group-data-[collapsible=icon]:w-[var(--sidebar-width-icon)] group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)] group-data-[layout=col]:top-[var(--topbar-height)] group-data-[layout=col]:h-[calc(100svh-var(--topbar-height))] group-data-[mode=overlay]:top-0 group-data-[mode=overlay]:h-full group-data-[side=left]:border-r group-data-[mode=static]:group-data-[layout=row]:group-data-[variant=inset]:border-none group-data-[layout=col]:md:!p-0 group-data-[mode=overlay]:md:!p-0 group-data-[state=collapsed]:md:!p-0 md:group-data-[state=expanded]:group-data-[variant=floating]:p-2 group-data-[variant=inset]:md:p-2"
    >
      <div
